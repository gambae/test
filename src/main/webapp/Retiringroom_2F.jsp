...
<body>
	<%
		...
		String location = "Retiringroom(2F)";
	...
	<form action="etc_ReservationCon" method="post">
		...
		<% if (dao.repetitionCheck(vo.getId(),date) == 0) { %>
			<input type="submit" value="예약하기" formaction="etc_ReservationCon?location=Retiringroom(2F)&date=<%=date %>">
		<% } else { %>
			<input type="button" value="예약하기" onclick="alert('이미 예약하셨습니다.')">
		<% } %>
		
		<a href="etc_ReservationSelectCon?location=Retiringroom(2F)&date=<%=date %>">예약 명단</a>
